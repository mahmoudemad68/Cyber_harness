...
| `packages/core/tools/src/index.ts` `schemas` / `requestSdkSchemas` | `schemas(scope)` is the public live enumeration of currently visible tools (parameters detached). `requestSdkSchemas` reads the assemble snapshot, drops `run_code`, and adds `output`. Nested SDK binding names come from that snapshot, not from public `schemas()`. |
...
| `packages/core/tools/src/ptc.ts` | SDK bindings are `requestSchemas(exec.agent)` minus `run_code`, a registry-private view of the assemble snapshot. Each binding closes over that enumerated `name` and passes it to `scheduler.prepare` / `executionMode`. Nested calls set `parent: exec.token`, so they bypass `collapses`. Start/settle events log that same `name` and the JSON-normalized arguments. |
...
- `ToolRuntime` owns live public enumeration (`schemas`), request-bound SDK/PTC projection, the assemble waterfall over `assembly.tools`, and reverse resolution (`resolveExecution` / `createExecution`). One assemble captures the mapping the request-bound paths share.
...
`assemble()` collects tool providers first (`wireSchemas`), then renders section text (including PTC SDK `requestSdkSchemas`), then runs the `system-prompt/assemble` waterfall (`projectAssembledTools`). The loop logs `request/header` from that assembly's `tools` and later copies `block.name` into `createExecution` / `executionMode`.
...
- `project` is host-only. Parameters on the returned schema are the registered object, never a rewritten copy. `ToolRuntime` snapshots the mapping during `assemble` for that scope; reverse resolution, nested SDK bindings, and concurrency classification use that snapshot until the next assemble. Public `schemas()` always projects the live visible set. Direct execute without assemble projects the live visible set.
