...
`wireSchemas` still contributes registered names so `toolOrder` matches them. A `system-prompt/assemble` listener then rewrites ToolRuntime-owned tools to exposed names. `wireSchemas` also snapshots that mapping for the assemble scope; reverse resolution, nested SDK bindings, concurrency classification, and assembled request tools use the snapshot until the next assemble. Public `schemas()` always projects the live visible set. Direct execute without assemble projects the live visible set.

Reverse resolution accepts only exposed names from that snapshot. `ToolExecution.name` is the registered name. `requestedName` is set only when it differs. After `createExecution`, dispatch reverse-resolves `requestedName ?? name` because the registered name is absent from the exposed set when a mapping renamed the tool. Policy, guards, concurrency classification, restrictions, and tool bodies keep the registered name. Adapters copy the assembled `ToolSchema` fields; they do not rename tools.
...
**Requiring `project()` to be pure instead of snapshotting.** Reverse resolution would still call `project()` after assemble, so a dispose, replacement, or stateful `project()` can disagree with the advertised map. The assemble snapshot is the request freeze.

## Consequences

A plugin on an agent or preset scope may call `registerSurface` from `apply`. Disposing the fiber restores identity for later assemblies. An in-flight request keeps the mapping captured at its assemble. Hidden and restricted tools cannot be reached through an alias. `standard` is unchanged.

## Testing

`packages/core/tools/tests/model-tool-surface.spec.ts` pins identity, projection, reverse resolution, disposal, nearest-scope, `toolOrder`, collisions, hidden and restricted tools, concurrency, cancellation, and request-bound freeze (dispose after assemble, stateful `project()`, hidden/restricted names, live `schemas()` after register/unregister, next assemble). `packages/core/tools/tests/ptc.spec.ts` pins native/ptc/both with an alias, including frozen SDK bindings after dispose and after a later register. `packages/core/agent-loop/tests/model-tool-surface.spec.ts` is the scripted loop proof: the model calls the exposed name, one registered tool runs, `request/header` records the exposed schemas, and a later request sees the post-dispose mapping. DeepSeek and OpenAI-compatible serializers copy the exposed name and parameters unchanged.
