...
`wireSchemas` 仍贡献注册名称，以便 `toolOrder` 与之匹配。随后 `system-prompt/assemble` 监听器把 ToolRuntime 拥有的工具改写为对外名称。`wireSchemas` 还会为该 assemble 作用域快照这次映射；反向解析、嵌套 SDK 绑定、并发分类与已组装的请求工具使用该快照，直到同作用域的下一次 assemble。公开的 `schemas()` 始终按当时的可见集合投影。未经 assemble 的直接 execute 按当时的可见集合投影。
...
`packages/core/tools/tests/model-tool-surface.spec.ts` 钉住恒等、投影、反向解析、释放、最近作用域、`toolOrder`、冲突、隐藏与被限制的工具、并发、取消，以及请求绑定的冻结（assemble 之后 dispose、有状态的 `project()`、隐藏/被限制名称、注册/注销后的实时 `schemas()`、下一次 assemble）。`packages/core/tools/tests/ptc.spec.ts` 钉住带别名的 native/ptc/both，包括 dispose 之后以及随后注册之后仍冻结的 SDK 绑定。`packages/core/agent-loop/tests/model-tool-surface.spec.ts` 是脚本化循环证明：模型调用对外名称，一个注册工具运行，`request/header` 记录对外 schema，且后续请求看到 dispose 之后的映射。DeepSeek 与 OpenAI 兼容序列化器原样复制对外名称和 parameters。
