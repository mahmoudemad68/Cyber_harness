...
`AgentPresets.registerRoot(root)` appends one existing `PresetRoot` to a live list on the existing service. The scanned order is the shipped root when included, then `config.roots`, then contributed roots in registration order, then the derived user-authored root when included. `list()` and `resolve()` remain unmemoized filesystem reads of that live list. `copy()`, `remove()`, and `readDocument()` capture that list once at entry and use it for resolution, collision checks, writable-root selection, and mutation, so a contribution registered or disposed while that call is in flight cannot retarget it.
...
Package contributors use `trust: 'system'` unless they intend to become the writable root. `writableRoot` still picks the first `user` root in the list captured at the start of that authoring call.
...
`packages/preset/agent-presets/tests/contributed-roots.spec.ts` pins ordering, disposal, Cordis plugin and Loader composition, unmemoized filesystem reads, authoring, standing-generation survival, and mount leak refusal through a contributed root. `packages/preset/agent-presets/tests/authoring-root-snapshot.spec.ts` pins that registering or disposing a contribution during an in-flight `copy` / `remove` / `readDocument` cannot change that call's source view, duplicate check, writable destination, or deletion target, while a later `list()` / `resolve()` still sees the live list. Existing shipped-root, user-root, discovery, and mount suites remain the no-contribution baseline.
