...
`ctx.tools.registerSurface(surface)` 声明从注册工具名到模型所见名称的作用域映射。默认是恒等（无声明），现有行为保持不变。`project` 可以隐藏工具，或替换其名称和描述；parameters 保持为注册 schema。重复的对外名称会失败。注册表在策略与执行之前，把模型提供的名称反向解析到注册名称。不同的参数 schema 应做成单独的类型化适配工具，而不是改写别名参数。
